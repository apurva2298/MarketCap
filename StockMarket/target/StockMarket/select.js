...
var app = angular.module("StockManagement", ["ui.grid"]);
    ...
    var savedColDefs = [
		...
		{name:'high',width:75,enableFiltering:false},
		{name:'low',width:75,enableFiltering:false},
		{name:'ytdChange',width:200,enableFiltering:false},
    ...
    var AllcolDefs = [
		...
		{name: 'symbol',displayName:'Symbol',width:95,enableColumnMenus:false,enableSorting:false},
		{name: 'companyName',width:300,enableColumnMenus:false,enableSorting:false},
		{name: 'sector',displayName:'Sector',width:150,enableFiltering:false},
                    ...
                    $http({
                      ...
                      $scope.allArr=[{}];//change to all array
                      	...
                      	for(var i=0;i<allCaps.length;i++)
                        {
                      	...
                      	$scope.gridOptions.columnDefs=AllcolDefs;
                      	$scope.gridOptions.excessRows=200;
                       $scope.gridOptions.data=$scope.allArr; //setting all cap data in grid*/
				 ...
				 $scope.getCompanyData=function(companyNm){
		               	 ...
		               	 $http({
		                     ...
		                     document.getElementById('myID').innerHTML="KEY STOCK DATA:<br><p>Company Name :"+$scope.stocks.companyName+
		                    	 "</p><p>Symbol :"+$scope.stocks.symbol+
		                    	 "</p><p>Sector :"+$scope.stocks.sector+
		                    	 "</p><p>Market Cap :"+$scope.stocks.marketCap+
					             "</p><p>Open :"+$scope.stocks.open+
						         "</p><p>Close :"+$scope.stocks.close+
							     "</p><p> PE Ratio:"+$scope.stocks.peRatio+
		                    	"</p><p>Previous Close :" +$scope.stocks.previousClose+
		                    	"</p><p>Calculation Price :" +$scope.stocks.calculationPrice+
		                    	 "</p>";
